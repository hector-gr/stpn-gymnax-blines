...
class BatchManager:
    def __init__(
        ...
        self.carry_synaptic_size = carry_synaptic_size
    ...
    def reset(self):
        return {
            ...
            "carry_hidden": jnp.empty(
                (self.n_steps, self.num_envs, *self.carry_size),
                dtype=jnp.float32,
            ),
            "carry_synaptic": jnp.empty(
                (self.n_steps, self.num_envs, *self.carry_synaptic_size),
                dtype=jnp.float32,
            ),
            "actions": jnp.empty(
    ...
    @partial(jax.jit, static_argnums=0)
    def append(self, buffer, state, carry, action, reward, done, log_pi, value):
        return {
                "states":  buffer["states"].at[buffer["_p"]].set(state),
                "carry_hidden":  buffer["carry_hidden"].at[buffer["_p"]].set(carry[0]),
                "carry_synaptic":  buffer["carry_synaptic"].at[buffer["_p"]].set(carry[1]),
    ...
    def get(self, buffer):
        ...
        batch = (
            buffer["states"][:-1],
            buffer["carry_hidden"][:-1],
            buffer["carry_synaptic"][:-1],
            buffer["actions"][:-1],
...
class RolloutManager(object):
    def __init__(self, model, env_name, env_kwargs, env_params):
        ...
        self.carry_size = (model.num_hidden_units,)
        self.initialize_carry = model.initialize_carry
    ...
    def select_action_ppo(
        ...
        value, pi, carry = policy(train_state.apply_fn, train_state.params, obs, carry, rng)
        action = pi.sample(seed=rng)
        log_prob = pi.log_prob(action)
        return action, log_prob, value[:, 0], carry, rng

    @partial(jax.jit, static_argnums=0)
    def batch_reset(self, keys):
        return self.initialize_carry(keys[0], self.observation_space.shape), jax.vmap(self.env.reset, in_axes=(0, None))(
            keys[1:], self.env_params
        )
    ...
    def batch_evaluate(self, rng_input, train_state, num_envs):
        ...
        carry_mem, (obs, state) = self.batch_reset(jax.random.split(rng_reset, num_envs + 1))

        def policy_step(state_input, _):
            """lax.scan compatible step transition in jax env."""
            obs, carry_mem, state, train_state, rng, cum_reward, valid_mask = state_input
            rng, rng_step, rng_net = jax.random.split(rng, 3)
            action, _, _, carry_mem, rng = self.select_action(train_state, obs, carry_mem, rng_net)
...
def policy(
    ...
    value, pi, carry = apply_fn(params, obs, carry, rng)
    return value, pi, carry
...
def train_ppo(rng, config, model, params, mle_log):
    ...
    batch_manager = BatchManager(
        ...
        carry_size=rollout_manager.carry_size,
        carry_synaptic_size=(model.num_hidden_units, model.num_hidden_units + rollout_manager.observation_space.shape[0]),
    ...
    def get_transition(
        ...
        action, log_pi, value, carry, new_key = rollout_manager.select_action(
            train_state, obs, carry, rng
        )
        ...
        batch = batch_manager.append(
            batch, obs, carry, action, reward, done, log_pi, value
        )
        return train_state, next_obs, carry, next_state, batch, new_key

    batch = batch_manager.reset()

    rng, rng_step, rng_reset, rng_eval, rng_update, rng_carry = jax.random.split(rng, 6)
    carry, (obs, state) = rollout_manager.batch_reset(
        jax.random.split(rng_reset, config.num_train_envs + 1)
    )
    ...
    for step in t:
        train_state, obs, carry, state, batch, rng_step = get_transition(
            train_state,
            obs,
            carry,
            state,
...
def loss_actor_and_critic(
    ...
    apply_fn: Callable[..., Any],
    obs: jnp.ndarray,
    carry_hidden: jnp.ndarray,
    carry_synaptic: jnp.ndarray,
    target: jnp.ndarray,
    ...
    value_pred, pi, carry = apply_fn(params_model, obs, (carry_hidden, carry_synaptic), rng=None)
...
def update(
    ...
    obs, carry_hidden, carry_synaptic, action, log_pi_old, value, target, gae = batch
    ...
    for _ in range(epoch_ppo):
        ...
        train_state, total_loss = update_epoch(
            ...
            idxes_list,
            flatten_dims(obs),
            flatten_dims(carry_hidden),
            flatten_dims(carry_synaptic),
            flatten_dims(action),
...
def update_epoch(
    ...
    for idx in idxes:
        ...
        total_loss, grads = grad_fn(
            ...
            obs=obs[idx],
            carry_hidden=carry_hidden[idx],
            carry_synaptic=carry_synaptic[idx],
            target=target[idx],
            value_old=value[idx],
