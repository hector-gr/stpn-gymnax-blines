...
class BatchManager:
    ...
    def append(self, buffer, state, carry, action, reward, done, log_pi, value):
        return {
                ...
                "carry_cell":  buffer["carry_cell"].at[buffer["_p"]].set(carry[0]),
                "carry_hidden":  buffer["carry_hidden"].at[buffer["_p"]].set(carry[1]),
...
class RolloutManager(object):
    ...
    def batch_reset(self, keys):
        return self.initialize_carry(keys[0]), jax.vmap(self.env.reset, in_axes=(0, None))(
...
def loss_actor_and_critic(
    ...
    value_pred, pi, carry = apply_fn(params_model, obs, (carry_cell, carry_hidden), rng=None)
...
def update(
    ...
    obs, carry_cell, carry_hidden, action, log_pi_old, value, target, gae = batch
